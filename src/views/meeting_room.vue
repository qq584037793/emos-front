<template>
  <div>
    <el-form :inline="true" :model="dataForm" :rules="dataRule" ref="dataForm">
      ...
      <el-form-item>
        <el-select v-model="dataForm.canDelete" class="input" placeholder="条件" size="medium">
          ...
          <el-option label="削除(可)" value="true"/>
          <el-option label="削除(不可)" value="false"/>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    >
      ...
      <el-table-column prop="name" header-align="center" align="center" min-width="150" label="会議室名"/>
      <el-table-column header-align="center" align="center" min-width="120" label="人数上限">
        <template #default="scope">
          <span>{{ scope.row.max }}人</span>
        </template>
      </el-table-column>
      <el-table-column header-align="center" align="center" min-width="100" label="状態">
        <template #default="scope">
          <span>{{ scope.row.status == 1 ? '使用可' : '使用不可' }}</span>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    deleteHandle: function (id) {
      ...
      if (ids.length == 0) {
        ...
        that.$confirm('選択した記録を削除してもよろしいですか？', {
