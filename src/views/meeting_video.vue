          ...
          会議中は、会場を静かにしてください.
			...
			<el-col :span="5">
				<el-card shadow="never">
					<template #header>
						<div class="card-header"><span>ユーザーリスト</span></div>
					</template>
...
export default {
    ...
    phoneHandle: function() {
      ...
      TRTC.checkSystemRequirements().then(checkResult => {
        if (!checkResult.result) {
          that.$alert('現在のブラウザはオンラインビデオ会議をサポートしていません', {
          ...
          if (that.meetingStatus) {
            ...
            that.client.unpublish(stream).then(() => {
              ...
              that.client
                  ...
                  .catch(error => {
                    console.error('会議室を正常に終了できませんでした' + error);
                  });
            });
          }
        }
    ...
    videoHandle: function() {
      let that = this;
      if (that.shareStatus) {
        that.$alert('画面共有中はカメラの電源をオンまたはオフにできません', {
          confirmButtonText: 'YES'
        });
        return;
      }
    ...
    shareHandle: function() {
      ...
      //判断用户是否进入视频会议室
      if (!that.meetingStatus) {
        that.$alert('画面を共有するには、最初にビデオ会議に参加してください', {
          confirmButtonText: 'YES'
        });
        return;
      }
      ...
      if (!TRTC.isScreenShareSupported()) {
        //提示当前浏览器不支持在线视频会议
        this.$alert('現在のブラウザは画面共有をサポートしていません',  {
          confirmButtonText: 'YES'
        });
        return;
      }
      ...
      if (that.shareStatus) {
        ...
        shareStream
            .initialize()
            .catch(error => {
              console.error('共有ストリームが失敗しました ' + error);
            })
            .then(() => {
              //取消推送本地视频流
